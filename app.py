...
def api_track_fun():
		...
		try:
			...
			push_key = request.args.get('pushkey')
			...
			db.child("Users").child(user_target).child(push_key).child("GSM Location").set(data)
			...
			statuserr = location_object['status']
			balanceerr = location_object['balance']
			messageerr = location_object['message']
			...
			data = {"Status": str(status)}
			data['Balance'] = str(balance)
			data['Message'] = str(messageerr)
			db.child(user_target).child(push_key).child("GSM Location").set(data)
